...
public class UnitsMove {
	...
	public UnitsMove(UnitsToMove unitsToMove, WorldView worldView) {
		...
		this.targetSector = worldView.getWorldSector(unitsToMove.getTargetCoords());
	...
	public void init() {
		...
		this.moveSeen = new MoveSeen(unitsToMove.getNation(), sectorDaoService, unitDaoService);
	}
	
	private boolean checkUnknown() {
		SectorSeen sectorSeen = sectorDao.findSectorSeen(unitsToMove.getNation(), targetSector);
		return (sectorSeen == null || attack.isEnemyCity());
	}
	/**
	 * @return
	 * for move: success if at least one unit made it to the target and there was no interception / interdiction
	 * for attack: same as move, but also at least one unit attacked
	 */
	...
	private Result<MoveCost> moveAndAttack() {
		...
		result = moveUnits();
		if (result.isSuccess() && attack.isAttackable(unitsToMove.getAttackType())) {
		...
		return new Result<MoveCost>(result.getMessages(), result.isSuccess(), new MoveCost(moveType), result.getBattleLogs(), result.isMoveSuccess());
	}
	...
	private Result<MoveCost> launchRocket() {
		...
		LaunchRocket launchRocket = unitCommandFactory
				.getLaunchRocket(worldView, firstUnit, unitsToMove.getTargetCoords(), moveSeen);
	...
	private Result<None> moveUnits() {
		...
		if (path == null) {
			return new Result<None>("Unable to find a path from "
					+ unitsToMove.getFirstCoords() + " to " + unitsToMove.getTargetCoords(), false);
		}
		if (path.isEmpty()) {
			// Already there
	...
	private Result<None> moveUnitsAlongPath(List<SectorCoords> path) {
		...
		for (SectorCoords coords : path) {
			...
			sectorDaoService.updateSeen(worldView, unitsToMove.getUnits(), moveSeen);
			moveSeen.persistSeen(); // Intermediate persist so enemies can see me to interdict me
			...
			if (attackType == AttackType.INITIAL_ATTACK || attackType == AttackType.INTERDICTION) {
				...
				if (!retval.isSuccess()) {
					if (unitsToMove.actorIsNation()) {
						...
						retval.addMessage("Interdicting "+unitsToMove.getNation()+" naval bombers halted by your fighters.");
					}
					break;
				}
			}
	...
	private void filterOutOfRangeUnits(Result<None> moveResult) {
		Result<List<Unit>> unitsOutOfRangeResult = unitsToMove.getUnitsOutOfRange(worldView, targetSector, unknown);
		...
		if (unitsToMove.isEmpty()) {
			...
			if (unitsOrdered.size() == 1) {
				moveResult.addMessage("Unit out of range.  Setting move order on unit.");
			} else if (unitsOrdered.size() > 1 ){
				moveResult.addMessage("All units out of range.  Setting move order on units.");
			} else {
	...
	private boolean couldMoveTo(Unit unit) {
		...
		if (result.isSuccess()) {
			result.and(movement.canEnter(unitsToMove.getAttackType(), targetSector,
					unit, unknown));
		}
	 ...
	 * @param range 
	 * @return
	 * success means no interdiction
	 */
	private Result<None> interdict(SectorCoords coords, SectorCoords excludeCoords) {
		...
		Unit ship = unitsToMove.getFirstUnit();
		Interdiction interdiction = unitCommandFactory.getInterdiction(ship, excludeCoords);
	...
	private Result<None> intercept(SectorCoords coords) {
		...
		Interception interception = unitCommandFactory.getInterception(unitsToMove,
				coords);
	...
	private Result<None> moveUnitsOneSector(SectorCoords coords) {
		...
		WorldSector worldSector = worldView.getWorldSector(coords);
		for (Unit unit : unitsToMove) {
			// Refresh this sector view in case the target sector is a transport filling up
			worldSector = sectorDaoService.refreshWorldSector(unit.getNation(),
					worldView, worldSector);
			UnitMoves unitMoves = unitCommandFactory.getUnitMoves(unitsToMove.getAttackType(),
	...
	private Result<MoveType> attack() {
		...
		for (Unit unit : unitsToMove) {
			...
			targetSector = sectorDaoService.refreshWorldSector(unitsToMove.getNation(),
			...
			if (!attack.isAttackable(unitsToMove.getAttackType())) {
				if (targetSector.isPlayerCity()) {
					result
							.addMessage("Target sector captured.  Not moving any more units.");
				} else {
					result
							.addMessage("All enemy units defeated.  Not moving any more units.");
				}
				break;
			}
		...
		return new Result<MoveType>(result.getMessages(), result.isSuccess(), moveType, result.getBattleLogs(), result.isMoveSuccess());
	}
	...
	private Result<None> attackWith(Unit unit) {
		UnitAttacksSector unitAttacker = unitCommandFactory
				.getUnitAttacksSector(unitsToMove.getActor(), unitsToMove.getAttackType(), unit, targetSector,
						worldView, moveSeen);
	...
	private List<SectorCoords> getPath() {
		for (Unit unit : unitsToMove) {
			Movement movement = new Movement(unit, worldView);
			List<SectorCoords> path = movement.getPath(unitsToMove.getAttackType(), unit,
