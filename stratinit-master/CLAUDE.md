...
**Bot players:** Server-side AI players that can be added from the game lobby via `POST /stratinit/add-bot`. Bots call domain services directly (no REST/HTTP/JWT). Games with bots start immediately (skip 24-hour scheduling delay). Bot turns execute periodically via `BotTurnEvent` (every 5 minutes, blitz-scaled). The AI uses a **Utility AI** pattern with configurable weights in `bot-weights.json` (designed for future RL tuning). Key classes: `BotService` (join/create), `BotExecutor` (orchestration), `BotActionGenerator` (candidate generation), `BotWorldState` (snapshot), `BotWeights` (tunable parameters). Eleven action types spanning all unit categories: city production (all 23 unit types gated by tech + coastal checks), land expansion movement, build city, land attack, naval combat, transport loading, air strikes/bombing, satellite launch, ICBM launch, city defense, diplomacy. `BotExecutor` tracks `usedUnitIds` to prevent the same unit from acting twice per turn. `BotWorldState` pre-computes coastal city coords and enemy cities for efficient decision-making. Bots are skipped for email notifications and scoring. See [BOTS.md](BOTS.md) for full architecture details.
