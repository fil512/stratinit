...
**Authentication:** JWT tokens via `POST /stratinit/auth/login`. Player registration via `POST /stratinit/auth/register`. Account settings via `GET/PUT /stratinit/profile` (`ProfileController`). `JwtAuthenticationFilter` validates Bearer tokens. HTTP Basic still supported for backward compatibility. Stateless sessions.
