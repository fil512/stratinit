...
**Bot players:** Server-side AI players that can be added from the game lobby via `POST /stratinit/add-bot`. Bots call domain services directly (no REST/HTTP/JWT). Games with bots start immediately (skip 24-hour scheduling delay). Bot turns execute periodically via `BotTurnEvent` (every 5 minutes, blitz-scaled). The AI uses a **Utility AI** pattern with configurable weights in `bot-weights.json` (designed for future RL tuning). Key classes: `BotService` (join/create), `BotExecutor` (orchestration), `BotActionGenerator` (candidate generation), `BotWorldState` (snapshot), `BotWeights` (tunable parameters). Fourteen action types spanning all unit categories: city production (all 23 unit types gated by tech + coastal checks), frontier-based land/naval exploration (terrain-aware, prioritizes unscouted territory), neutral city capture, build city, land attack, naval combat, transport loading, transport destination planning (sail loaded transports to other islands), disembark (land units step off transports onto land), air strikes/bombing, satellite launch, ICBM launch, city defense, diplomacy. `BotExecutor` tracks `usedUnitIds` to prevent the same unit from acting twice per turn. `BotWorldState` respects fog-of-war: visible units come from `UnitDao.getSeenUnits()` (sight-range filtered) and enemy cities from `CityDao.getSeenCities()` (scouted sectors only). **Multi-unit coordination** emerges through utility bonuses: land units converging on the same target get `coordinationBonus`, air strikes where ground forces can follow up get `airSupportBonus`, and naval combat near friendly transports gets `navalEscortBonus`. `BotWorldState` provides coordination queries (`countLandUnitsInRangeOf`, `countAirUnitsInRangeOf`, `hasNearbyTransport`, `hasLandThreatNear`). Bots are skipped for email notifications and scoring. See [BOTS.md](BOTS.md) for full architecture details.
