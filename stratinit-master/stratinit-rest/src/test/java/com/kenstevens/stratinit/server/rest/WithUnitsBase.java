...
public abstract class WithUnitsBase extends BaseStratInitControllerTest {
    ...
    @Autowired
    protected UnitService unitServiceImpl;
    ...
    public void doJoinGame() {
        joinGamePlayerMe();
        testInfantry = unitServiceImpl.buildUnit(nationMe, START_COORDS, UnitType.INFANTRY);
        testInfantry.addMobility();
        testInfantry.addMobility();
        testInfantryId = testInfantry.getId();
        testHelicopter = unitServiceImpl.buildUnit(nationMe, START_COORDS, UnitType.HELICOPTER);
        unitDao.save(testHelicopter);
        testHelicopterId = testHelicopter.getId();
        unitServiceImpl.buildUnit(nationMe, SEA1, UnitType.TRANSPORT);

        unitServiceImpl.buildUnit(nationMe, SEA2, UnitType.TRANSPORT);
        for (int i = 0; i < 6; ++i) {
            testInfantries.add(unitServiceImpl.buildUnit(nationMe, SEA2, UnitType.INFANTRY));
        }
    }
